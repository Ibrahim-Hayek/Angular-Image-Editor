...
import { getTransformationsFromExifData, supportsAutomaticRotation } from '../utilities/exif.utils';
import { resizeCanvas } from '../utilities/resize.utils';
import { ExifTransform } from '../interfaces/exif-transform.interface';
import { HammerStatic } from '../utilities/hammer.utils';
...
export class ImageCropperComponent implements OnChanges, OnInit {
    ...
    private activatePinchGesture(): void {
        if (this.Hammer) {
            ...
            let h;
            h = hammer;
            h.on('pinchmove', this.onPinch.bind(this));
            h.on('pinchend', this.pinchStop.bind(this));
            h.on('pinchstart', this.startPinch.bind(this));
        } else if (isDevMode()) {
